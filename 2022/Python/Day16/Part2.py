...
def main():
    ...
    your_score, path = get_max_score(valve_map, 'AA', max_time=26)
    for valve in valve_map.values():
        for visited in path[1:]:
            if visited in valve.neighbors:
                valve.neighbors.pop(visited)

    elephants_score, elephants_path = get_max_score(valve_map, 'AA', max_time=26)

    print(your_score + elephants_score)
